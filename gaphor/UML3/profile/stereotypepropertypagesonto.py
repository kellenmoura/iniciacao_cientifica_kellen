...
@PropertyPages.register(UML3.Element)
class StereotypeRelatorPage(PropertyPageBase):
    ...
    def construct(self):
        ...
        stereotypes = UML3.model.get_stereotypes(subject)
        if not stereotypes:
            return None
        ...
        model, toggle_stereotyperelator_handler, set_slot_value_handler = stereotyperelator_model(
            subject
        )

        stereotyperelator_list = builder.get_object("stereotyperelator-list")
        stereotyperelator_list.set_model(model)

        builder.connect_signals(
            {
                "show-stereotypes-changed": (self._on_show_stereotypes_change,),
                "toggle-stereotype": toggle_stereotyperelator_handler,
...
def stereotyperelator_model(subject):
    ...
    return model, (_toggle_stereotyperelator, subject, model), (_set_value, model)


def refresh(subject, model):
    stereotypes = UML3.model.get_stereotypes(subject)
    instances = subject.appliedStereotypeRelator
...
@transactional
def _toggle_stereotyperelator(renderer, path, subject, model):
    row = model[path]
    name, old_value, is_applied, _, _, stereotyperelator, _, _ = row
    value = not is_applied

    if value:
        UML3.model.apply_stereotyperelator(subject, stereotyperelator)
    else:
        UML3.model.remove_stereotyperelator(subject, stereotyperelator)
...
def _set_value(renderer, path, value, model):
    ...
    if isinstance(attr, UML3.StereotypeRelator):
    ...
    if slot is None:
        slot = UML3.model.add_slot(applied, attr)
