...
@represents(UML3.Subcollectionof)
class SubcollectionofItem(LinePresentation[UML3.Subcollectionof], Named):
    ...
    def __init__(self, diagram, id=None):
        ...
        self.shape_middle = Box(
            Text(
                text=lambda: stereotypes_str(self.subject, ("Subcollectionof",)),
            ),
            Text(text=lambda: self.subject.name and ""),
        )
...
class AssociationEnd:
    ...
    def __init__(self, owner: SubcollectionofItem, end: Optional[str] = None):
        self._canvas = None
        self._owner = owner
        self._end = end
    ...
    @property
    def owner(self) -> SubcollectionofItem:
